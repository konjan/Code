	...
	void Update ()
	{
		if (isActive == true)
		{
			if (m_GM.GameMode == Mode.BuildMode)
			{
				...
				transform.parent.position = new Vector3(Mathf.Round((ppos.x) / SnapSize) * SnapSize, transform.position.y, Mathf.Round((ppos.z) / SnapSize) * SnapSize);

				if (Input.GetButtonDown("RTrigger"))
					this.transform.RotateAround(transform.parent.position, Vector3.up, 90);
